...
def create_pipeline(**kwargs) -> Pipeline:
    ...
    if not models_dir.exists():
        print(f"⚠️ No se encontró la carpeta de modelos: {models_dir}. No se realizará ninguna evaluación.")
        return Pipeline([])
    ...
    if not model_keys:
        print(f"⚠️ No se encontraron modelos para evaluar en {models_dir}. No se realizará ninguna evaluación.")
        return Pipeline([])
    ...
    for key in model_keys:
        if "_" not in key:
            print(f"⚠️ Formato inválido en el nombre del modelo: {key}. Se omite.")
            continue

        model_type, combo_id = key.split("_", 1)
        model_ds  = f"models.{run_id}.{key}"                # input model
        output_ds = f"evaluation.{run_id}.{key}_output"            # output results
        tag       = key                                     # tag / param_key
    ...
    if not subpipelines:
        print("⚠️ No se procesaron subpipelines de evaluación debido a la falta de modelos válidos.")
        return Pipeline([])

    print(f"✅ Total de subpipelines de evaluación: {len(subpipelines)}")
