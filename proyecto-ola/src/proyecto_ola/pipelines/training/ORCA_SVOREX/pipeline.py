...
def create_pipeline(**kwargs) -> Pipeline:
    ...
    dataset_name = f"ORCA_SVOREX_C_{C}_epsilon_{epsilon}_max_iter_{max_iter}_run_{run_id}".replace(".", "")

    return Pipeline(
        [
            node(
                func=ORCA_SVOREX,  # La función ORCA_SVOREX que devolverá el modelo vacío
                ...
                name=f"ORCA_SVOREX_node_C_{C}_epsilon_{epsilon}_max_iter_{max_iter}_run_{run_id}"
            )
        ]
    )   